...
// some memory for the region list
#define N_REGIONS 128
#define N_AREAS 128

static struct vrm_region_lists r_ls;
static struct vrm_region       regions[N_REGIONS];
static struct vrm_area         areas[N_AREAS];
...
static void fetch_regionlist(void);
...
#define DEBUG_MYSELF 0
  ...
  VG_(strncpy)(buf, vrm_get_filename(fd), nbuf);

#if DEBUG_MYSELF
  VG_(debugLog)(0, "aspacemgr", "## %s: filename for fd %d is %s\n", __func__, fd, buf);
#endif
...
static 
#endif
Int allocate_segname ( const HChar* name )
{
   Int i, j, len;
...
static void show_nsegment ( Int logLevel, Int segNo, NSegment* seg )
{
   ...
   switch (seg->kind) {
      ...
      case SkAnonC: case SkAnonV: case SkShmC:
         VG_(debugLog)(
            logLevel, "aspacem",
            "%3d: %s %010llx-%010llx %s %c%c%c%c%c N: %08lx\n",
            segNo, show_SegKind(seg->kind),
            (ULong)seg->start, (ULong)seg->end, len_buf,
...
void VG_(am_show_nsegment) ( Int logLevel, Int segNo, NSegment *seg )
{
  show_nsegment(logLevel, segNo, seg);
}
/* Print out the segment array (debugging only!). */
...
static Bool sane_NSegment ( NSegment* s )
{
   ...
   if (s->start >= s->end) {
      if (0) VG_(debugLog)(1,"aspacem", "s->start >=s->end\n");
      return False;
   }
   ...
   if (!VG_IS_PAGE_ALIGNED(s->start)) {
      if (0) VG_(debugLog)(1,"aspacem", "%s: s->start is no page aligned\n", __func__);
      return False;
   }
   if (!VG_IS_PAGE_ALIGNED(s->end+1)) {
      if (0) VG_(debugLog)(1,"aspacem", "%s: s->end+1 is not page aligned\n", __func__);
      return False;
   }
   ...
   switch (s->kind) {
      ...
      case SkFree:
         if (0) VG_(debugLog)(1,"aspacem", "%s: case SkFree\n", __func__);
         return 
            s->smode == SmFixed
      ...
      case SkAnonC: case SkAnonV: case SkShmC:
         if (0)
             VG_(debugLog)(1,"aspacem", "%s: case SkAnon\n", __func__);
         return 
            s->smode == SmFixed 
            && s->dev == 0 && s->ino == 0 && s->offset == 0 && s->fnIdx == -1
      ...
      case SkFileC: case SkFileV:
         if (0) VG_(debugLog)(1,"aspacem", "%s: case SkFile\n", __func__);
         return 
            s->smode == SmFixed
            && (s->fnIdx == -1 ||
      ...
      case SkResvn: 
         if (0) VG_(debugLog)(1,"aspacem", "%s: case SkResvn\n", __func__);
         return 
            s->dev == 0 && s->ino == 0 && s->offset == 0 && s->fnIdx == -1 
            && !s->hasR && !s->hasW && !s->hasX && !s->hasT
            && !s->isCH;
      ...
      default:
         if (0) VG_(debugLog)(1,"aspacem", "%s: case default\n", __func__);
         return False;
   }
}
...
#if defined(VGO_l4re)
             /*
              * On L4Re, we can only merge anonymous memory, if they are
              * backed by the same dataspace (which potentially never happens?)
              */
             && s1->dsNodePtr && s2->dsNodePtr && s1->dsNodePtr == s2->dsNodePtr
#endif
       ) {
            s1->end = s2->end;
            s1->hasT |= s2->hasT;
                             ...
                             const HChar* file, Int line )
{
   ...
   parse_regionlist( sync_check_mapping_callback,
...
void VG_(am_set_nodeptr)(NSegment* const seg, Addr const ptr)
{
    seg->dsNodePtr = ptr;
}
...
Addr VG_(am_startup) ( Addr sp_at_startup )
{
   ...
   /* To determine maxAddr, we ask the region manager */

   fetch_regionlist();
   aspacem_minAddr = 0x1000;
   aspacem_maxAddr = r_ls.max_addr;
...
#  endif
   ...
   parse_regionlist( read_maps_callback, NULL );
                            ...
                            /*OUT*/Bool* ok )
{
   ...
   */
   ...
   if (req->len == 0) {
      if (1) {
         VG_(debugLog)(0,"aspacm", "%s: reject zero-length request\n", __func__);
      }
      *ok = False;
      return 0;
   }
   ...
   if ((req->rkind==MFixed || req->rkind==MHint)  
      ...
      if (1) {
         VG_(debugLog)(0,"aspacm", "%s: reject wraparound\n", __func__);
      }
      *ok = False;
      return 0;
   }
   ...
   if (forClient && req->rkind == MFixed) {
      ...
      *ok = False;
      if (1) {
         VG_(debugLog)(0,"aspacm", "%s: Policy Exception #1 not acceptable\n", __func__);
      } 
      return 0;
   }
   ...
   switch (req->rkind) {
      case MFixed:
         if (fixedIdx >= 0) {
            *ok = True;
            ...
            if (1) {
              VG_(debugLog)(0,"aspacm", "%s: Default policy not acceptable; MFixed\n", __func__);
            }
            *ok = False;
            return 0;
         }
      ...
      case MAny:
         ...
         if (1) {
            VG_(debugLog)(0,"aspacm", "%s: Default policy not acceptable; MAny\n", __func__);
         }
         *ok = False;
         return 0;
      default:
...
/* Notifies aspacem that the valgrind completed an mmap successfully.
   The segment array is updated accordingly.  If the returned Bool is
   True, the caller should immediately discard translations from the
   specified address range. */

Bool
VG_(am_notify_valgrind_mmap)( Addr a, SizeT len, UInt prot, UInt flags,
                            Int fd, Off64T offset )
{
   HChar    buf[VKI_PATH_MAX];
   NSegment seg;
   ...
   init_nsegment( &seg );
   seg.kind   = (flags & VKI_MAP_ANONYMOUS) ? SkAnonV : SkFileV;
   seg.start  = a;
   seg.end    = a + len - 1;
   seg.hasR   = toBool(prot & VKI_PROT_READ);
   seg.hasW   = toBool(prot & VKI_PROT_WRITE);
...
#  if !VGO_l4re
   ...
   add_segment( &seg );
#else
   /* In L4Re the NO_NOTIFY part is actually bogus as we use the segment array
    * for region management and thus now already have a segment set up for this
    * region. Therefore we simply look it up and adapt it a bit.
    */
   NSegment *s = VG_(am_find_nsegment)(sr_Res(sres));
   s->kind   = SkFileC;
   s->offset = offset;
   s->hasR   = toBool(prot & VKI_PROT_READ);
   s->hasW   = toBool(prot & VKI_PROT_WRITE);
   s->hasX   = toBool(prot & VKI_PROT_EXEC);
...
#  if !VGO_l4re
    ...
    * region. Therefore we simply look it up and adapt it a bit.
    */
   NSegment *seg = VG_(am_find_nsegment)(sr_Res(sres));
   seg->kind = SkAnonC;
#endif
...
SysRes VG_(am_mmap_anon_float_valgrind)( SizeT length )
{
   ...
   if (!vcap_running) {
       ...
       seg.hasW  = True;
       seg.hasX  = True;
       vrm_segment_fixup(&seg);
       add_segment( &seg );
#if defined(VGO_l4re)
   } else {
       NSegment *s = VG_(am_find_nsegment)(sr_Res(sres));
       if (!s) {
           VG_(printf)("Could not find segment that should be existing!\n");
		   enter_kdebug("error");
	   }
       s->kind  = SkAnonV;
       s->hasR  = True;
                                  ...
                                  ShrinkMode smode, SSizeT extra )
{
   ...
   seg.kind  = SkResvn;
   if (0) {
      VG_(debugLog)(0, "aspacem", "%s: setting segment.kind to SkResvn\n", __func__);
      show_nsegment_full(0, &seg);
   }
...
static void fetch_regionlist(void) {
   VG_(memset)(&r_ls, 0, sizeof(struct vrm_region_lists));
   VG_(memset)(regions, 0, sizeof(struct vrm_region) * N_REGIONS);

   r_ls.regions = regions;
   r_ls.areas = areas;

   vrm_get_lists(&r_ls, N_REGIONS, N_AREAS);
}
   ...
   )
{
   ...
   UChar *filename = NULL;
   dev = ino = 0;
   if (0) l4re_rm_show_lists();

   VG_(debugLog)(1, "main", "region_count = %d, area_count = %d\n", r_ls.region_count, r_ls.area_count);
   VG_(debugLog)(1, "main", "Region maping: limits [%lx-%lx]\n", r_ls.min_addr, r_ls.max_addr);

   gapStart = Addr_MIN;

   VG_(debugLog)(1, "main", " Area map:\n");
   for (i = 0; i < r_ls.area_count; i++) 
     VG_(debugLog)(1, "main", "  [%10lx-%10lx] -> flags=%x\n", r_ls.areas[i].start, r_ls.areas[i].end, r_ls.areas[i].flags);

   VG_(debugLog)(1, "main", " Region map:\n");
   
   for (i = 0; i < r_ls.region_count; i++) {
     start = r_ls.regions[i].start;
     endPlusOne = r_ls.regions[i].end + 1;

     // TODO offset from ds not from region
     foffset = 0; //r_ls.regions[i].offset;
     
     prot = 0;
 
     prot |= VKI_PROT_READ;
     if ((r_ls.regions[i].flags | 1) == 1)
        prot |= VKI_PROT_WRITE;
     prot |= VKI_PROT_EXEC;
     
     if (start == L4RE_KIP_ADDR)
       prot = VKI_PROT_READ | VKI_PROT_WRITE | VKI_PROT_EXEC;
       
     VG_(debugLog)(1, "main", "  [%10lx-%10lx] -> (offs=%lx, ds=%lx, flags=%x)\n", 
                   r_ls.regions[i].start,  r_ls.regions[i].end,  
                   r_ls.regions[i].offset_in_ds,  r_ls.regions[i].ds, 
                   r_ls.regions[i].flags);
     ...
     if (record_mapping && start < endPlusOne) {
         (*record_mapping) ( start, endPlusOne-start,
                             prot, dev, ino,
                             foffset, filename );
         vrm_segment_notify(r_ls.regions[i].start,  r_ls.regions[i].end,  r_ls.regions[i].ds, r_ls.regions[i].offset_in_ds, r_ls.regions[i].flags);
     }
