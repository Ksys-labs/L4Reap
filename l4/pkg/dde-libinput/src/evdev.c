...
int dde_evdev_open(int i)
{
	struct evdev_client *list;

	if (!evdev_table[i] || !evdev_table[i]->exist)
		return -ENODEV;

	if (!(list = kzalloc(sizeof(struct evdev_client), GFP_KERNEL)))
		return -ENOMEM;

	list->evdev = evdev_table[i];
	list_add_tail(&list->node, &evdev_table[i]->client_list);
