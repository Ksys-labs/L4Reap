...
#if !defined(VGO_l4re) // L4Re uses it outside as well
static
#endif
...
#ifdef VGO_l4re
      case VEX_TRC_JMP_L4_UTCB_EAX:   return "L4UTCB EAX";
      case VEX_TRC_JMP_L4_UTCB_EBX:   return "L4UTCB EAX";
      case VEX_TRC_JMP_L4_UTCB_ECX:   return "L4UTCB EAX";
      case VEX_TRC_JMP_L4_UTCB_EDX:   return "L4UTCB EAX";
...
#if defined(VGO_l4re)
         VG_(debugLog)(1, "sched", "Initializing ThreadState %d\n", i);

         VG_(memset)(ts_utcb_copy(&VG_(threads)[i]), 0, L4RE_UTCB_SIZE);
...
{
#if defined(VGO_l4re)
//   VG_(unimplemented)("unimplemented function block_signals()");
   /* Do nothing */
#else
   vki_sigset_t mask;
...
#if defined(L4RE_DEBUG_EXECUTION)
       //VG_(get_and_pp_StackTrace)( tid, VG_(clo_backtrace_size) );
       VG_(debugLog)(0, "sched","tid=%d, ip=%p, bbs_done=%lld\n", tid, (void *)ip, bbs_done);
//           (VG_(translate)( tid, ip, /*debug*/True, 1 /*0xffffffff*/ /*0*//* verbose*/,
...
#define DEBUG_UTCB \
         if (1) \
            VG_(debugLog)(0, "sched", "utcb access via client " \
                                      "virtual utcb of thread %d @ %p -> eax\n", \
                                      tid, \
                                      (Addr)ts_utcb(&VG_(threads)[tid]));
#endif
#define TRACK_UTCB \
         VG_TRACK( new_mem_startup, (Addr)ts_utcb(&VG_(threads)[tid]), L4RE_UTCB_SIZE, 1, 1, 1, 0 );

      case VEX_TRC_JMP_L4_UTCB_EAX:
         VG_(threads)[tid].arch.vex.guest_EAX = (Addr)ts_utcb(&VG_(threads)[tid]);
         DEBUG_UTCB
         TRACK_UTCB
         break;
      case VEX_TRC_JMP_L4_UTCB_EBX:
         VG_(threads)[tid].arch.vex.guest_EBX = (Addr)ts_utcb(&VG_(threads)[tid]);
         DEBUG_UTCB
         TRACK_UTCB
         break;
      case VEX_TRC_JMP_L4_UTCB_ECX:
         VG_(threads)[tid].arch.vex.guest_ECX = (Addr)ts_utcb(&VG_(threads)[tid]);
         DEBUG_UTCB
         TRACK_UTCB
         break;
      case VEX_TRC_JMP_L4_UTCB_EDX:
         VG_(threads)[tid].arch.vex.guest_EDX = (Addr)ts_utcb(&VG_(threads)[tid]);
         DEBUG_UTCB
         TRACK_UTCB
         break;
      case VEX_TRC_JMP_L4_UTCB_EDI:
         VG_(threads)[tid].arch.vex.guest_EDI = (Addr)ts_utcb(&VG_(threads)[tid]);
         DEBUG_UTCB
         TRACK_UTCB
         break;
      case VEX_TRC_JMP_L4_UTCB_ESI:
         VG_(threads)[tid].arch.vex.guest_ESI = (Addr)ts_utcb(&VG_(threads)[tid]);
         DEBUG_UTCB
         TRACK_UTCB
         break;
...
#if defined(VGO_l4re)
         /* In case of l4re int 0x3 means enter_kdebug().*/
         handle_syscall(tid, trc);
...
#        elif defined(VGP_x86_l4re)
         /* nearly the same. L4Re stores return EIP in %ebx */
         VG_(threads)[tid].arch.vex.guest_EIP
            = VG_(threads)[tid].arch.vex.guest_EBX;
         handle_syscall(tid, trc);
#        else
         vg_assert2(0, "VG_(scheduler), phase 3: "
