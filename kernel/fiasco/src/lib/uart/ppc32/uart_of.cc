...
namespace L4 {
	...
	bool Uart_of::startup(unsigned long) 
	{
		...
		for(phandle_t node = 0; prom_next_node(&node); ) {
			...
			if(prom_call("package-to-path", 3, 1, node, path, sizeof(path)) 
			   == PROM_ERROR) 
				return false;
			
			/* open port */
			unsigned prom_ret;
			if((prom_ret = prom_call("open", 1, 1, path)) == PROM_ERROR)
				return false;
			_serial = (ihandle_t)prom_ret;
			break;
		}
