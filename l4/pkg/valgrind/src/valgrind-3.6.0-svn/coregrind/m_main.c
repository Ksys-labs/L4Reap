...
Int _valgrind_main(Int argc, char** argv, char** envp);
...
{
   int i, loglevel;
   const char *toolname = VG_(toolname);
   ...
   for (i = 1; i < argc; i++) {
      ...
      if (0 == VG_(strcmp)(argv[i], "-d")) 
         loglevel++;
         // in l4re tool are started directly - so parsing cmdline for
         // toolname is not necessary
//      if (0 == VG_(strncmp)(argv[i], "--tool=", 7))
//         toolname = argv[i] + 7;
   }
   ...
   if (toolname) {
      VG_(debugLog)(1, "launcher", "tool '%s' requested\n", toolname);
        ...
        VG_(debugLog)(1, "launcher",
                         "no tool requested, defaulting to '%s'\n", toolname);
  }

   platform = "x86-l4re";

   return valgrind_main(argc, argv, envp);
}
#endif
...
#if defined(VGO_l4re)
   l4_utcb_tcr()->user[2] = 1; // thread id
   toolname = VG_(toolname);
#endif
...
#  endif
...
#if defined(VGO_l4re)
   //--------------------------------------------------------------
   // Start the L4Re virtual region mapper, which interposes all
   // memory management and therefore makes sure that VG always
   // knows about which areas and regions in memory are mapped
   // and by whom.
   //--------------------------------------------------------------
   VG_(debugLog)(1, "main", "Starting L4Re VCap thread.\n");
   l4re_vcap_start_thread();
#endif
   //============================================================
   //
   ...
   //--------------------------------------------------------------
   // Since there is no implementation of exec in L4Re, we must start
   // the tools direcly.
#if !defined(VGO_l4re)
   ...
   //--------------------------------------------------------------
   // L4Re doesn't support set/getrlimit()...
#if !defined(VGO_l4re)
...
#if defined(VGO_l4re)
   VG_(client_rlimit_stack).rlim_cur=8*1024*10424;
   VG_(client_rlimit_stack).rlim_max=64*1024*1024;
...
#     if defined(VGO_linux) || defined(VGO_darwin) || defined(VGO_l4re)
      the_iicii.argv              = argv;
      the_iicii.envp              = envp;
...
#if defined(VGO_l4re)
   // for clients to make systemcalls, they require access to the L4 KIP
   {
     NSegment * seg = (NSegment *)VG_(am_find_nsegment)( (Addr) L4RE_KIP_ADDR );
     vg_assert(seg);
     seg->kind = SkAnonC;
   }

   {
     NSegment * seg = (NSegment *)VG_(am_find_nsegment)( (Addr) L4RE_KERNEL_START );
     vg_assert(seg);
     seg->kind = SkAnonC;
   }
#endif
...
#     if defined(VGP_x86_linux) || defined(VGP_x86_l4re)
      iters = 5;
...
#  if defined(VGO_linux) || defined(VGO_l4re)
   { Addr* seg_starts;
     Int   n_seg_starts;
     Addr_n_ULong anu;
...
#if defined(VGO_l4re)
   { Bool change_ownership_v_c_OK;
     Addr co_start   = VG_PGROUNDDN( (Addr)&VG_(l4re_trampoline_stuff_start) );
     Addr co_endPlus = VG_PGROUNDUP( (Addr)&VG_(l4re_trampoline_stuff_end) );
     VG_(debugLog)(1,"redir",
                     "transfer ownership V -> C of 0x%llx .. 0x%llx (l4re_trampoline_stuff)\n",
                     (ULong)co_start, (ULong)co_endPlus-1 );
...
#if defined(VGO_l4re)
    VG_TRACK( new_mem_startup, (Addr) client_env, client_env_size,  1, 1, 1, 0 );
#endif
   //--------------------------------------------------------------
...
static void final_tidyup(ThreadId tid)
{
#if !defined(VGO_darwin) && !defined(VGO_l4re)
...
/* If linking of the final executables is done with a libc present,
   then Valgrind starts at main() above as usual, and all of the
   following code is irrelevant.

   However, this is not the intended mode of use.  The plan is to
   avoid linking against a libc, by giving gcc the flags 
   -nodefaultlibs -lgcc -nostartfiles at startup.
...
/* The kernel hands control to _start, which extracts the initial
   stack pointer and calls onwards to __c_main.  This switches to
   the new stack and calls then _start_in_C_l4re. */
/* _start is implemented in coregrind/l4re/__c_main.c */
...
/* Avoid compiler warnings: this fn _is_ used, but labelling it
   'static' causes gcc to complain it isn't. */
void _start_in_C_l4re ( int argc, char **argv, char **envp ,int esp);
...
void _start_in_C_l4re ( int argc, char **argv, char **envp ,int esp)
{
   ...
   VG_(memset)( &the_iicii, 0, sizeof(the_iicii) );
   VG_(memset)( &the_iifii, 0, sizeof(the_iifii) );
//   the_iicii.sp_at_startup = 0x20000;

   the_iicii.sp_at_startup = (Addr)&argc;
   r = _valgrind_main( (Int)argc, argv, envp );
   /* NOTREACHED */
   VG_(exit)(r);
}
