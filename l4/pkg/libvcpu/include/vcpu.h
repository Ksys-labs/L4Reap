...
L4_INLINE
void
l4vcpu_irq_enable(l4_vcpu_state_t *vcpu,
                   l4vcpu_event_hndl_t do_event_work_cb) L4_NOTHROW;
...
L4_INLINE
void
l4vcpu_halt(l4_vcpu_state_t *vcpu,
            l4vcpu_event_hndl_t do_event_work_cb) L4_NOTHROW;
...
void
l4vcpu_irq_enable(l4_vcpu_state_t *vcpu,
                   void (*do_event_work_cb)(l4_vcpu_state_t *vcpu)) L4_NOTHROW
{
  while (1)
    {
      vcpu->state |= L4_VCPU_F_IRQ;
      ...
      if (!(vcpu->sticky_flags & L4_VCPU_SF_IRQ_PENDING))
        break;

      l4vcpu_irq_disable(vcpu);
      vcpu->i.tag = l4_ipc_wait(l4_utcb(), &vcpu->i.label, L4_IPC_NEVER);
      if (!l4_msgtag_has_error(vcpu->i.tag))
...
{
  if (s & L4_VCPU_F_IRQ)
    l4vcpu_irq_enable(vcpu, do_event_work_cb);
}
