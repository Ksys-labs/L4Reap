...
ESZ(Addr) mapelf(struct elfinfo *e, ESZ(Addr) base)
{
   ...
   for (i = 0; i < e->e.e_phnum; i++) {
      ...
      if (VG_PGROUNDUP(bss)-VG_PGROUNDDN(addr) > 0) {
         if (0) {
             VG_(debugLog)(0,"ume","mmap_file_fixed_client #1\n");
             VG_(debugLog)(0, "ume", "mmap(%p, %lx, %lx, %d, %lx)\n", VG_PGROUNDDN(addr),
                           VG_PGROUNDUP(bss)-VG_PGROUNDDN(addr), prot, e->fd, VG_PGROUNDDN(off));
             if (0) VG_(am_show_nsegments)(0,"before #1");
         }
