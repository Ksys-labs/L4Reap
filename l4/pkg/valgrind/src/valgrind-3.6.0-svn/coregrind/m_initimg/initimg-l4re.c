...
#else
   exe_name = VG_(args_the_exename);
#endif

   VG_(memset)(info, 0, sizeof(*info));
   ret = VG_(do_exec)(exe_name, info);
   VG_(debugLog)(1, "initimg", "do_exec = %d\n", ret);
   if (ret < 0) {
...
#if defined(VGO_l4re) && !defined(AT_L4RE_ENVPAGE)
#define AT_L4RE_ENVPAGE 0xf1
#endif
...
#else
             resvn_size - L4RE_STACKSIZE - inner_HACK,
#endif
             SmUpper, 
#if !VGO_l4re
             anon_size + inner_HACK
#else
             anon_size + L4RE_STACKSIZE + inner_HACK
#endif
          );
     if (ok) {
        ...
        res = VG_(am_mmap_anon_fixed_client)(
             ...
             anon_size +inner_HACK,
#else
             anon_start - L4RE_STACKSIZE - inner_HACK,
             anon_size  + L4RE_STACKSIZE + inner_HACK,
#endif
...
#  endif
   ...
   if (info->phdr != 0) {
       ...
       /* AT_PHDR tells the interpreter where to find the program header table in the memory image */
       auxv->a_type = AT_PHDR;
       auxv->u.a_val = info->phdr;
       auxv++;

       /* AT_PHNUM number of entries in the program header table at which the AT_PHDR entry points */
       if (0) VG_(printf)("info->phnum = %p\n", info->phnum);
       auxv->a_type = AT_PHNUM;
       auxv->u.a_val = info->phnum;
       auxv++;

       /* AT_BASE base address at which the interpreter program was loaded into memory */
       if (0) VG_(printf)("info->interp_base = %p\n", info->interp_base);
       auxv->a_type = AT_BASE;
       auxv->u.a_val = info->interp_base;
       auxv++;

       /* AT_ENTRY entry point of the application program to which the interpreter program should transfer control */
       if (0) VG_(printf)("info->entry = %p\n", info->entry);
...
#if defined(VGO_l4re)
         case AT_L4RE_ENVPAGE:
            /*
             * We found an aux pointer containing info about an
             * L4Re env pointer. We modify the original env by
             * replacing certain caps we are interested in with
             * the cap of a vcap thread.
             */
            {
                VG_(debugLog)(0, "initimg",
                               "\033[31ml4re_global_env found\033[0m\n");
...
IIFinaliseImageInfo VG_(ii_create_image)( IICreateImageInfo iicii )
{
   ...
   //--------------------------------------------------------------
   VG_(debugLog)(1, "initimg", "\033[31;1mLoading client\033[0m\n");
...
#if VGO_l4re
   vg_assert((void*)iifii.initial_client_IP != NULL);
#endif
