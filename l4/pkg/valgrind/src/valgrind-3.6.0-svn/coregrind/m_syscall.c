...
SysRes VG_(mk_SysRes_x86_l4re) ( UInt val ) {
   SysRes res;
   res._isError = val >= -4095 && val <= -1;
   if (res._isError) {
      res._err = -val;
...
UWord do_l4re_syscall_WRK( SyscallArgs* args )
{
   struct { unsigned d,v; } v = {args->arg5, args->arg6};
...
UWord do_l4re_syscall_debug_WRK( SyscallArgs* args )
{
   struct { unsigned d,v; } v = {args->arg5, args->arg6};
...
UWord handle_int80(SyscallArgs *args)
{
   struct { unsigned d,v; } v = {args->arg5, args->arg6};
    __asm__ __volatile__ (
         "      push  %%ebp         \n\t"
...
UWord do_l4re_syscall_enter_kdebug_WRK( SyscallArgs* args )
{
   struct { unsigned d,v; } v = {args->arg5, args->arg6};
   __asm__ __volatile__ (
         "      int   $0x3            \n\t"
...
UWord handle_ud2(SyscallArgs *a)
{
    __asm__ __volatile__ (
        "ud2\n\t"
    );
...
#if defined(VGO_l4re)
static void dump_args(char *str, SyscallArgs *args, l4_utcb_t *u, ThreadId tid)
{
//      l4_msg_regs_t *v = l4_utcb_mr_u(l4_utcb_wrap());
//      l4_msg_regs_t *v = l4_utcb_mr_u(VG_(threads)[VG_(get_running_tid)()].utcb);

      l4_msg_regs_t *v = l4_utcb_mr_u(u);

      VG_(debugLog)(0, "syscall", "%s\n", str);
      VG_(debugLog)(0, "syscall", "Registers:\n"
                                  "\t\t sysno = %08lx\n" 
                                  "\t\t    a1 = EAX = 0x%08x  a1 = EBX = 0x%08x\n"
                                  "\t\t    a2 = ECX = 0x%08x  a3 = EDX = 0x%08x\n"
                                  "\t\t    a4 = ESI = 0x%08x  a5 = EDI = 0x%08x\n"
                                  "\t\t    a6 = EBP = 0x%08x\n",
                                  (int)args->sysno, (int)args->arg1,
                                  (int)args->arg2,  (int)args->arg3,
                                  (int)args->arg4,  (int)args->arg5,
                                  (int)args->arg6,  (int)args->arg7);


      VG_(debugLog)(0, "syscall", "UTCB contents:\n"
                                  "\t\t         MR0 = 0x%08x MR1 = 0x%08x\n"
                                  "\t\t         MR2 = 0x%08x MR2 = 0x%08x\n"
                                  "\t\t         MR4 = 0x%08x MR4 = 0x%08x\n"
                                  "\t\t         MR6 = 0x%08x MR6 = 0x%08x\n"
                                  "\t\t         MR8 = 0x%08x MR8 = 0x%08x\n",
                                  (unsigned int) v->mr[0], (unsigned int) v->mr[1],
                                  (unsigned int) v->mr[2], (unsigned int) v->mr[3],
                                  (unsigned int) v->mr[4], (unsigned int) v->mr[5],
                                  (unsigned int) v->mr[6], (unsigned int) v->mr[7],
                                  (unsigned int) v->mr[8], (unsigned int) v->mr[9]);
}

static void dump_backtrace(char *str, ThreadId tid)
{
      VG_(debugLog)(0, "syscall", "Backtrace of client:\n", str);
      VG_(get_and_pp_StackTrace)( tid, VG_(clo_backtrace_size) );
}


/* Handling for enter_kdebug(text)                 done
 *              outchar(char)                      done
 *              outstring(const char *)            done
 *              outnstring(const char *, unsigned) done
 *              outhex32(int)                      todo
 *              outhex20(int)                      todo
 *              outhex16(int)                      todo
 *              outhex12(int)                      todo
...
void handle_kdebug(SyscallArgs *args, ThreadId tid) {
#define MAX_STR_LEN 128
#define DEBUG_MYSELF 0
    char str[MAX_STR_LEN];
    Int len = -1;

    char const *s1 = "\033[35m"; // color magenta
    char const *s2 = "\033[0m";  // color reset

    unsigned char *ip;
    ip = VG_(threads)[tid].arch.vex.guest_EIP;
#if DEBUG_MYSELF
    //VG_(debugLog)(0, "syscall", "%02X %02X %02X %02X\n", *ip, *(ip+1), *(ip+2), *(ip+3));
    l4re_hexdump(ip, 16);
#endif 
    str[0] = 0;
    if (*(ip) == 0xEB /* jmp */) {
        /* enter_kdebug(text) */
        Int len = *(ip+1);
        vg_assert(len >= 0 && len < MAX_STR_LEN);

        VG_(strncpy)(&str, ip + 2, len);
        str[len] = 0;

        VG_(printf)("%senter_kdebug via client: %s%s\n", s1,  str, s2);
        VG_(get_and_pp_StackTrace)( tid, VG_(clo_backtrace_size) );
        do_l4re_syscall_enter_kdebug_WRK(args);
    } else if (*(ip) == 0x3C /* cmp */) {
        switch(*(ip + 1)) {
            case 0x0: /* outchar(char) */
                {
                    Char c = VG_(threads)[tid].arch.vex.guest_EAX & 0x000000ff;
                    VG_(printf)("%s%c%s", s1, c , s2);
                    break;
                }
            case 0x1: /* outnstring(const char*, unsigned) */
                {
                    char *_str = VG_(threads)[tid].arch.vex.guest_EAX;
                    unsigned len = VG_(threads)[tid].arch.vex.guest_ECX;
                    vg_assert(len >= 0 && len < MAX_STR_LEN);
                    VG_(strncpy)(&str, _str, len);
                    str[len] = 0;
                    VG_(printf)("%s%s%s", s1, str , s2);
                    break;
                }
            case 0x2: /* outstring(const char*) */
                {
                    char *_str = VG_(threads)[tid].arch.vex.guest_EAX;
                    VG_(printf)("%s%s%s", s1, _str , s2);
                    break;
                }
            case 0x5: /* outhex32(int) */
                {
                    int n = VG_(threads)[tid].arch.vex.guest_EAX;
                    VG_(printf)("%s%x%s", s1, n, s2);
                    break;
                }
            case 0xB /* 11 */: /* outdec(int) */
                {
                    int n = VG_(threads)[tid].arch.vex.guest_EAX;
                    VG_(printf)("%s%d%s", s1, n, s2);
                    break;
                }
            ...
            default:
                {
                    VG_(printf)("%sunknown outXY()%s\n", s1, s2);
                    VG_(debugLog)(0, "syscall", "%x %x %x %x\n", *ip, *(ip+1), *(ip+2), *(ip+3));
                    do_l4re_syscall_enter_kdebug_WRK(args);
                }
        }
    } else { /* everything else */
        VG_(printf)("%senter_kdebug via client%s\n", s1, s2);
        do_l4re_syscall_enter_kdebug_WRK(args);
    }
...
Bool dbg_artificial_handling = DEBUG_OFF;

void handle_artificial(SyscallArgs *args)
{
    L4RedirState *s = (L4RedirState *)args->arg1;
    switch(s->redir_type) {
        case LIBC_OPEN:
            if (dbg_artificial_handling) VG_(debugLog)(4, "systrp", "open(file='%s' flags=%08lx) = %d\n",
                                               s->open.path, s->open.flags, s->retval);
            vrm_register_filename(s->retval, s->open.path);
            break;
        case LIBC_CLOSE:
            if (dbg_artificial_handling) VG_(debugLog)(4, "systrp", "close(fd=%d) = %d\n",
                                               s->close.fd, s->retval);
            vrm_unregister_filename(s->close.fd);
            break;
        case LIBC_MMAP:
            {
                if (dbg_artificial_handling) {
                    VG_(debugLog)(4, "systrp", "mmap(a=%08lx, size = %lx, prot = %08lx\n",
                                  s->mmap.addr, s->mmap.size, s->mmap.prot);
                    VG_(debugLog)(4, "systrp", "     fd = %d offset = %lx) = %lx\n",
                                  s->mmap.fd, s->mmap.offset, s->retval);
                }

                /* Modify segment -> SkFileC */
                if (s->mmap.fd >= 0) {
                    extern Int allocate_segname(const HChar *);

                    HChar buf[VKI_PATH_MAX];

                    /*
                     * The segment has already been registered (as anon client)
                     * by VRM. Now find it and adapt its type.
                     */
                    NSegment *seg = VG_(am_find_nsegment)(s->retval);
                    seg->kind = SkFileC;

                    /*
                     * Also, store file offset and adapt the mapping's
                     * access rights properly.
                     */
                    seg->offset = s->mmap.offset;
                    seg->hasR   = toBool(s->mmap.prot & VKI_PROT_READ);
                    seg->hasW   = toBool(s->mmap.prot & VKI_PROT_WRITE);
                    seg->hasX   = toBool(s->mmap.prot & VKI_PROT_EXEC);

                    VG_(strncpy)(buf, vrm_get_filename(s->mmap.fd), VKI_PATH_MAX);
                    seg->fnIdx = allocate_segname(buf);

                    /*
                     * Finally, tell the debug info reader to check if
                     * it needs information from this.
                     */
                    VG_(di_notify_mmap)(s->mmap.addr, s->mmap.size);
                }
            }
            break;
        default:
            VG_(printf)("Unknown artificial redir type: %lx\n", s->redir_type);
            enter_kdebug("unknown artificial redir\n");
            break;
    }
}


SysRes VG_(do_l4re_syscall) ( SyscallArgs *args, ThreadId tid )
{
#define DEBUG_MYSELF 0
    l4_utcb_t *utcb = l4_utcb_wrap();

    // we havn't the big look - so malloc is a bad idea!!
    //void *vg_utcb = VG_(malloc)("vg_utcb", 512);
    char _vg_utcb[L4RE_UTCB_SIZE];
    void *vg_utcb = &_vg_utcb[0];

#if DEBUG_MYSELF
    VG_(debugLog)(0, "syscall", "\033[36mThe client makes a syscall tid = %d\033[0m\n"
            "\t\t real-utcb @ %p virt-utcb @ %p\n",
            tid,
            (void *)utcb,
            ts_utcb(&VG_(threads)[tid]));

    dump_args("  -- Before syscall --", args, ts_utcb(&VG_(threads)[tid]), tid);
    dump_backtrace(" -- Before syscall --", tid);
#endif
    // backup valgrinds utcb
    VG_(memcpy)(vg_utcb, utcb, L4RE_UTCB_SIZE);


    // virt utcb --> real utcb
    VG_(memcpy)(utcb, ts_utcb(&VG_(threads)[tid]), L4RE_UTCB_SIZE);

    // do syscall for guest
    switch (args->sysno) {
        case SYS_INVOKE:
            do_l4re_syscall_WRK(args);
            break;
        case SYS_DEBUG:
            do_l4re_syscall_debug_WRK(args);
            break;
        case SYS_ENTER_KDEBUG:
            handle_kdebug(args, tid);
            break;
        case SYS_LINUX_INT80:
            handle_int80(args);
            break;
        case SYS_ARTIFICIAL:
            handle_artificial(args);
            break;
        case SYS_UD2:
            handle_ud2(args);
            break;
        default:
            VG_(printf)("unknown syscall: %lx\n", args->sysno);
            break;
    }

    // real utcb --> virt utcb
    VG_(memcpy)(ts_utcb(&VG_(threads)[tid]), utcb, L4RE_UTCB_SIZE);

    // restore valgrinds utcb
    VG_(memcpy)(utcb, vg_utcb, L4RE_UTCB_SIZE);


#if DEBUG_MYSELF
    dump_args("  -- After syscall --", args, ts_utcb(&VG_(threads)[tid]), tid);
#endif
...
#elif defined(VGO_l4re)
   VG_(unimplemented)("unimplemented function VG_(do_syscall)");
...
#     endif
      default:
#if defined(VGO_l4re)
                            VG_(debugLog)(1, "syscall", "%s: unknown errnum: %d\n", __func__, (int) errnum);
#endif
                            return "VG_(strerror): unknown error";
   }
}
