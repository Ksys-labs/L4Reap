...
static void l4x_x86_flush_thread(void)
{
	...
	//LOG_printf("l4x_x86_flush_thread: %s(%d) %d\n", tsk->comm, tsk->pid, tsk->thread.started);
	...
	tsk->thread.cloner = L4_INVALID_CAP;
#endif
}
...
{
	return do_fork(CLONE_VFORK | CLONE_VM | SIGCHLD, regs->sp, regs, COPY_THREAD_STACK_SIZE___FLAG_USER,
		       NULL, NULL);
}
	  ...
	  void __user *parent_tid, void __user *child_tid, struct pt_regs *regs)
{
	if (!newsp)
		newsp = regs->sp;
	return do_fork(clone_flags, newsp, regs, COPY_THREAD_STACK_SIZE___FLAG_USER, parent_tid, child_tid);
}
