...
#if defined(VGO_l4re)
      char utcb_store[L4RE_UTCB_SIZE]; // XXX rename utcb_copy ?
      l4_utcb_t *utcb;
      enum
      {
          SYS_INVOKE       = 0x23,
          SYS_DEBUG        = 0x24,
          SYS_ENTER_KDEBUG = 0x25,
		  SYS_LINUX_INT80  = 0x26,
		  SYS_UD2          = 0x27,
		  SYS_ARTIFICIAL   = 0x28,
      };
#endif
...
/*
 * L4Re redirections that trap into Valgrind even though they are not
 * real system calls.
 *
 * _dl_open, _dl_close, _dl_mmap
 */
enum RedirType
{
	LIBC_OPEN,
...
/*------------------------------------------------------------*/
/*--- L4Re OS state accessor functions                     ---*/
/*------------------------------------------------------------*/
   static inline char *ts_utcb_copy(ThreadState *ts)
   { return ts->os_state.utcb_store; }

   static inline l4_utcb_t *ts_utcb(ThreadState *ts)
   { return ts->os_state.utcb; }
#endif
