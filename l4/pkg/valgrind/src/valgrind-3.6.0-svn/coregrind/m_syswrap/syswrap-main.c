...
/* Useful info which needs to be recorded somewhere:
   ...
   L4Re:
   x86    sys eax  ebx  ecx  edx  esi  edi  ebp  n/a  eax
...
#else
   VG_(unimplemented)("unimplemented function do_syscall_for_client()");
#endif
}
                                    ...
                                    /*IN*/ UInt trc )
{
   ...
   VexGuestX86State* gst = (VexGuestX86State*)gst_vanilla;
   switch (trc) {
       case VEX_TRC_JMP_SYS_SYSENTER: // fallthrough
       case VEX_TRC_JMP_SYS_INT48:
           canonical->sysno = SYS_INVOKE;
           break;
       case VEX_TRC_JMP_SYS_INT50:
           canonical->sysno = SYS_DEBUG;
           break;
       case VEX_TRC_JMP_SIGTRAP:
           canonical->sysno = SYS_ENTER_KDEBUG;
           break;
	   case VEX_TRC_JMP_SYS_INT128:
		   canonical->sysno = SYS_LINUX_INT80;
		   break;
	   case VEX_TRC_JMP_L4_UD2:
		   canonical->sysno = SYS_UD2;
		   break;
	   case VEX_TRC_JMP_L4_ARTIFICIAL:
		   canonical->sysno = SYS_ARTIFICIAL;
		   break;
       default:
           VG_(printf)("Unhandled TRC: %d ??\n", trc);
           enter_kdebug("invalid syscall");
   }
   canonical->arg1  = gst->guest_EAX;
   canonical->arg2  = gst->guest_EBX;
   canonical->arg3  = gst->guest_ECX;
   canonical->arg4  = gst->guest_EDX;
   canonical->arg5  = gst->guest_ESI;
   canonical->arg6  = gst->guest_EDI;
...
#elif defined(VGP_x86_l4re)
   VexGuestX86State* gst = (VexGuestX86State*)gst_vanilla;
   gst->guest_EAX = canonical->arg1;
   gst->guest_EBX = canonical->arg2;
   gst->guest_ECX = canonical->arg3;
   gst->guest_EDX = canonical->arg4;
   gst->guest_ESI = canonical->arg5;
...
#  elif defined(VGO_l4re)
   if (0) VG_(message)(Vg_DebugMsg, "syscallno = %x = %d\n", syscallno, syscallno);
...
#if defined(VGO_l4re)
         // TODO should we not call do_syscall_for_client
         sres  = VG_(do_l4re_syscall)(&sci->args, tid);

         sci->status = convert_SysRes_to_SyscallStatus(sres);
         /* After a syscall on L4, every register can be modified by the kernel.
            Because of this, it is important to flush these changes back to the
            guest state. */
         putSyscallArgsIntoGuestState( &sci->args, &tst->arch.vex );
...
#if defined(VGO_l4re)
         SysRes sres = VG_(do_l4re_syscall)(&sci->args, tid);
         /* After a syscall on L4, every register can be modified by the kernel.
            Because of this, it is important to flush these changes back to the
            guest state. */
         putSyscallArgsIntoGuestState( &sci->args, &tst->arch.vex );
