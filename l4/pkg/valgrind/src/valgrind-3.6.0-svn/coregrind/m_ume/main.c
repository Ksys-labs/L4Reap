...
VG_(pre_exec_check)(const HChar* exe_name, Int* out_fd, Bool allow_setuid)
{
   ...
   if (sr_isError(res)) {
	   VG_(debugLog)(0, "m_ume", "error opening executable\n");
      return res;
   }
   fd = sr_Res(res);

   VG_(debugLog)(0, "m_ume", "exec fd = %d\n", fd);
   // Check we have execute permissions
   ret = VG_(check_executable)(&is_setuid, (HChar*)exe_name, allow_setuid);
   VG_(debugLog)(0, "m_ume", "executable: %s\n", ret ? "False" : "True");
   if (0 != ret) {
      VG_(close)(fd);
   ...
   VG_(debugLog)(0, "m_ume", "file size: %d\n", fsz);
   if (fsz < bufsz)
      bufsz = fsz;
...
Int VG_(do_exec_inner)(const HChar* exe, ExeInfo* info)
{
   ...
   VG_(debugLog)(1, "m_ume", "pre_exec_check(%s) = %d\n", exe, res._err);
   VG_(debugLog)(1, "m_ume", "is error: %s\n", sr_isError(res) ? "true" : "false");
   if (sr_isError(res))
      return sr_Err(res);
   ...
   VG_(debugLog)(1, "m_ume", "exe_handler @ %p\n",
                 exe_handlers[sr_Res(res)].load_fn);
...
Int VG_(do_exec)(const HChar* exe_name, ExeInfo* info)
{
   ...
   VG_(debugLog)(1, "m_ume", "do_exec_inner=%d\n", ret);
