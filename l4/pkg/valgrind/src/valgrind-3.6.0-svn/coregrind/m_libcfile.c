...
#  elif defined(VGO_aix5) || defined (VGO_l4re)
...
#define DEBUG_MYSELF 0
    // Instead of making a direct system call, in L4Re we call the library 
    // function direct.
    SysRes res; 
    Int ret; 
    void *addr;
    Int size;
#if DEBUG_MYSELF
//    VG_(am_show_nsegments)(1,"Memory layout before open");

    VG_(debugLog)(0, "libcfile", "## %s(%s, %d %s %s %s + %s, %x)\n",
            __func__, pathname, 
            flags, 
            (flags & VKI_O_RDONLY) ? "r" : "",  
            (flags & VKI_O_WRONLY) ? "w" : "",
            (flags & VKI_O_RDWR)   ? "rw" : "",
			(flags & VKI_O_L4RE_MAGIC) ? "mag" : "!mag",
			mode);
#endif
    if ((flags & VKI_O_WRONLY) || (flags & VKI_O_RDWR)) {
		// without a writeable filesystem this is not possible
		VG_(debugLog)(0, "libcfile",
		              "%s: file can only opened for reading at!!\n",
					  pathname);
		enter_kdebug("libcfile:open");
    }
    ...
    if ((ret = open(pathname, flags, mode)) < 0) {
		...
		if (flags & VKI_O_L4RE_MAGIC) {
#if 0
			new_node = VG_(create_node)();
			pathname_len = VG_(strlen)(pathname);
			new_node->filename = (char *) VG_(malloc)("libcfile", pathname_len + 1);
			VG_(strncpy)(new_node -> filename, pathname, pathname_len + 1);
			VG_(printf)("opened %s\n", new_node->filename);

			new_node->fd = ret;
			VG_(insert_node_right)(root_node, new_node);

			// workaround for mappings from the fprov lib
			int r = get_file_pos_and_size(ret, &addr, &size);
			vg_assert(r > -1);

#if DEBUG_MYSELF
			VG_(debugLog)(0, "libcfile", "## open file @ 0x%lx size 0x%lx\n", addr, size);
			l4re_rm_show_lists();
#endif
			VG_(am_notify_valgrind_mmap)((Addr) addr, VG_PGROUNDUP(size), VKI_PROT_READ,
					VKI_MAP_ANONYMOUS, ret, 0);
#endif
		}
    }

#if DEBUG_MYSELF
//    VG_(am_show_nsegments)(1,"Memory layout after open");
...
#  elif defined(VGO_l4re)
   // XXX: we probably got a file mapping here which we need
   //      to remove here
   close(fd);
   vrm_unregister_filename(fd);
#  else
#    error Unknown OS
#  endif
}
...
// l4re version, because uclibc has some field differently
#define TRANSLATE_libc_TO_vg_stat(_p_vgstat, _p_vkistat) \
   do { \
      (_p_vgstat)->dev        = (ULong)( (_p_vkistat)->st_dev ); \
      (_p_vgstat)->ino        = (ULong)( (_p_vkistat)->st_ino ); \
      (_p_vgstat)->nlink      = (ULong)( (_p_vkistat)->st_nlink ); \
      ...
      (_p_vgstat)->blksize    = (ULong)( (_p_vkistat)->st_blksize ); \
      (_p_vgstat)->blocks     = (ULong)( (_p_vkistat)->st_blocks ); \
      (_p_vgstat)->atime      = (ULong)( (_p_vkistat)->st_atim.tv_sec ); \
      (_p_vgstat)->atime_nsec = (ULong)( (_p_vkistat)->st_atim.tv_nsec ); \
      (_p_vgstat)->mtime      = (ULong)( (_p_vkistat)->st_mtim.tv_sec ); \
      (_p_vgstat)->mtime_nsec = (ULong)( (_p_vkistat)->st_mtim.tv_nsec ); \
      (_p_vgstat)->ctime      = (ULong)( (_p_vkistat)->st_ctim.tv_sec ); \
      (_p_vgstat)->ctime_nsec = (ULong)( (_p_vkistat)->st_ctim.tv_nsec ); \
   } while (0)
...
#  elif defined(VGO_l4re)
   {
       ...
       res._err = stat(file_name, &libc_buf);
       if (res._err < 0)
           res._isError = True;
       else {
           TRANSLATE_libc_TO_vg_stat(vgbuf, &libc_buf);
           res._isError = False;
       }
   }
...
#undef TRANSLATE_libc_TO_vg_stat /* l4re */
...
{
#  if defined(VGO_l4re)
   return access(path, (irusr ? 4/*R_OK*/ : 0) 
		 | (iwusr ? 2/*W_OK*/ : 0) 
		 | (ixusr ? 1/*X_OK*/ : 0)); 
...
#  elif defined(VGP_x86_darwin)
   ...
   return res;
#  elif defined(VGO_l4re)
   /* Get the old offset -> we need to restore this later */
   Int old_off = lseek(fd, 0, VKI_SEEK_CUR);

   /* move to offset */
   Int ret = lseek(fd, offset, VKI_SEEK_SET);
   if (ret < 0) {
	   res._isError = True;
	   res._err = ret;
   }
   ...
   /* reset offset */
   ret = lseek(fd, old_off, VKI_SEEK_SET);
...
#  elif defined(VGP_ppc32_aix5) || defined(VGP_ppc64_aix5) || defined(VGO_l4re)
   I_die_here;
